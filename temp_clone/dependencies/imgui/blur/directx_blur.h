...
void draw_background_blur(ImDrawList* draw_list, IDXGISwapChain* swap_chain, ID3D11Device* _device, ID3D11DeviceContext* _ctx, ImVec2 start, ImVec2 end, int rounding = 0, ImDrawFlags flags = 0) {
	...
	const ImVec2 uv_min(
		display_size.x > 0.0f ? (start.x / display_size.x) : 0.0f,
		display_size.y > 0.0f ? (start.y / display_size.y) : 0.0f
	);
	const ImVec2 uv_max(
		display_size.x > 0.0f ? (end.x / display_size.x) : 1.0f,
		display_size.y > 0.0f ? (end.y / display_size.y) : 1.0f
	);
	draw_list->AddImageRounded(tex, start, end, uv_min, uv_max, ImColor{ 1.f, 1.f, 1.f, ImGui::GetStyle().Alpha }, (float)rounding, flags);
