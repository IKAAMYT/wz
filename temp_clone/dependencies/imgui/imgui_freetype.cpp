...
#define FT_CEIL(X)  (((X + 63) & -64) / 64)
    ...
    void FreeTypeFont::SetPixelHeight(int pixel_height)
    {
        ...
        req.width = 0;
        req.height = (uint32_t)(pixel_height * 64 * Scale);
        ...
        Info.PixelHeight = (uint32_t)(pixel_height * InvScale);
        Info.Ascender = (float)FT_CEIL(metrics.ascender) * InvScale;
        Info.Descender = (float)FT_CEIL(metrics.descender) * InvScale;
        Info.LineSpacing = (float)FT_CEIL(metrics.height) * InvScale;
        Info.LineGap = (float)FT_CEIL(metrics.height - metrics.ascender + metrics.descender) * InvScale;
        Info.MaxAdvanceWidth = (float)FT_CEIL(metrics.max_advance) * InvScale;
    }
...
bool ImFontAtlasBuildWithFreeTypeEx(FT_Library ft_library, ImFontAtlas* atlas, unsigned int extra_flags)
{
    ...
    for (int src_i = 0; src_i < src_tmp_array.Size; src_i++)
    {
        ...
        for (int glyph_i = 0; glyph_i < src_tmp.GlyphsCount; glyph_i++)
        {
            ...
            // Register glyph
            float x0 = info.OffsetX * src_tmp.Font.InvScale + font_off_x;
            float y0 = info.OffsetY * src_tmp.Font.InvScale + font_off_y;
            float x1 = x0 + info.Width * src_tmp.Font.InvScale;
            float y1 = y0 + info.Height * src_tmp.Font.InvScale;
            float u0 = (tx) / (float)atlas->TexWidth;
            float v0 = (ty) / (float)atlas->TexHeight;
            float u1 = (tx + info.Width) / (float)atlas->TexWidth;
            float v1 = (ty + info.Height) / (float)atlas->TexHeight;
            dst_font->AddGlyph(&cfg, (ImWchar)src_glyph.Codepoint, x0, y0, x1, y1, u0, v0, u1, v1, info.AdvanceX* src_tmp.Font.InvScale);
