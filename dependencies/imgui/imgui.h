...
namespace ImGui
{
    ...
    IMGUI_API bool          SubButton(const char* label, bool selected, const ImVec2& size = ImVec2(0, 0));   // button
    ...
    IMGUI_API void          SetItemTooltip(const char* fmt, ...) IM_FMTARGS(1);                // set a text-only tooltip for current item if hovered
...
struct ImDrawList
{
    ...
    IMGUI_API void  AddRectFilledMultiColorRounding(const ImVec2& p_min, const ImVec2& p_max, ImU32 col_upr_left, ImU32 col_upr_right, ImU32 col_bot_right, ImU32 col_bot_left, float rounding = 0, ImDrawFlags flags = 0);
