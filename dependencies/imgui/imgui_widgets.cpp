...
bool ImGui::Keybind(const char* label, int* key, bool show_label)
{
    ...
    if (*key != 0 && g.ActiveId != id)
        strcpy_s(buf_display, CustomWidgets::KeyName(*key));
    ...
    it_anim->second.text = ImLerp(it_anim->second.text,
        (g.ActiveId == id) ? ui::colors::text : (ImGui::IsItemHovered() ? ui::colors::text : ui::colors::text_disabled),
        io.DeltaTime * 6.f);

    static std::map<ImGuiID, float> anim_width;
    auto it_width = anim_width.find(id);
    if (it_width == anim_width.end()) {
        anim_width[id] = 0.f;
        it_width = anim_width.find(id);
    }

    float base_width = label_size.x + 8.f;
    float active_width = base_width + 10.f;
    float target_width = (g.ActiveId == id) ? active_width : base_width;

    const float epsilon = 0.1f;
    float diff = fabs(it_width->second - target_width);
    if (diff < epsilon)
        it_width->second = target_width;
    else
        it_width->second = ImLerp(it_width->second, target_width, io.DeltaTime * 10.f);

    const float icon_space = 24.0f;

    ImVec2 rect_min = ImVec2(rect.Max.x - it_width->second, rect.Min.y);
    ImVec2 rect_max = rect.Max;
    ImVec2 full_rect_min = rect_min;
    full_rect_min.x -= icon_space;

    ImRect clickable(full_rect_min, rect_max);
    bool hovered = ItemHoverable(clickable, id);

    window->DrawList->AddRectFilled(full_rect_min, rect_max, GetColorU32(ui::colors::keybind::background), 3);

    ImU32 separator_col = GetColorU32(ui::colors::keybind::outline_background);
    window->DrawList->AddLine(ImVec2(rect_min.x, rect_min.y + 3), ImVec2(rect_min.x, rect_max.y - 3), separator_col);

    window->DrawList->AddRect(full_rect_min, rect_max, GetColorU32(ui::colors::keybind::outline_background), 3);

    if (show_label)
    {
        ImVec2 label_text_size = CalcTextSize(label);
        float rect_top = full_rect_min.y;
        float rect_bottom = rect_max.y;
        float rect_height = rect_bottom - rect_top;
        float label_pos_y = rect_top + (rect_height - label_text_size.y) * 0.5f;

        window->DrawList->AddText(ImVec2(rect.Min.x, label_pos_y), GetColorU32(it_anim->second.text), label);
    }
 
    float center_x = (rect_min.x + rect_max.x) * 0.5f;
    float center_y = (rect_min.y + rect_max.y) * 0.5f;

    ImVec2 text_pos = ImVec2(center_x - label_size.x * 0.5f, center_y - label_size.y * 0.5f - 1.f);
    window->DrawList->AddText(text_pos, GetColorU32(ui::colors::text), buf_display);

    ImGui::PushFont(font.widget_icon[0]);
    const char* icon_text = "E";
    ImVec2 icon_size = CalcTextSize(icon_text);
    ImVec2 icon_center = ImVec2(full_rect_min.x + icon_space * 0.5f, (rect_min.y + rect_max.y) * 0.5f);
    ImVec2 icon_pos = ImVec2(icon_center.x - icon_size.x * 0.5f + 1, icon_center.y - icon_size.y * 0.5f - 1);
    window->DrawList->AddText(icon_pos, GetColorU32(ui::colors::main), icon_text);
    ImGui::PopFont();

    ImGui::PopFont();

    struct keybind_capture_state {
        bool primed = false;
        bool ignore_mouse[5] = {};
        bool ignore_keys[256] = {};
    };
    static std::map<ImGuiID, keybind_capture_state> capture_state;
    auto it_capture = capture_state.find(id);
    if (it_capture == capture_state.end()) {
        capture_state.insert({ id, keybind_capture_state{} });
        it_capture = capture_state.find(id);
    }
    ...
    if (hovered && io.MouseClicked[0]) {
        ...
        // Snapshot currently-held inputs so only *new* presses can bind.
        it_capture->second.primed = true;
        for (int i = 0; i < 5; ++i) {
            it_capture->second.ignore_mouse[i] = io.MouseDown[i];
        }
        for (int vk = 0; vk < 256; ++vk) {
            it_capture->second.ignore_keys[vk] = (GetAsyncKeyState(vk) & 0x8000) != 0;
        }
    }
    ...
    if (g.ActiveId == id) {
        if (CustomWidgets::controller_capture_only) {
            int controller_key = 0;
            if (CustomWidgets::GetPressedControllerButton(&controller_key)) {
                k = controller_key;
                value_changed = true;
                ImGui::ClearActiveID();
                it_capture->second.primed = false;
            }
        }
        else {
            const int mouse_to_vk[5] = { VK_LBUTTON, VK_RBUTTON, VK_MBUTTON, VK_XBUTTON1, VK_XBUTTON2 };

            for (auto i = 0; i < 5; i++) {
                if (i == 0) {
                    // Never allow left-click to be bound from this widget.
                    continue;
                }
                const bool down_now = (GetAsyncKeyState(mouse_to_vk[i]) & 0x8000) != 0;
                if (!down_now) {
                    it_capture->second.ignore_mouse[i] = false;
                    continue;
                }
                if (!it_capture->second.primed || it_capture->second.ignore_mouse[i]) {
                    continue;
                }
                if (down_now) {
                    switch (i) {
            ...
            if (!value_changed) {
                for (int i = 0x08; i <= 0xFE; ++i) {
                    const bool down_now = (GetAsyncKeyState(i) & 0x8000) != 0;
                    if (!down_now) {
                        it_capture->second.ignore_keys[i] = false;
                        continue;
                    }
                    if (!it_capture->second.primed || it_capture->second.ignore_keys[i]) {
                        continue;
                    }
                    if (down_now) {
                        k = i;
...
void ImGui::keybind_popup(const char* str_id, const char* label, int* key, int* key_mode, bool* binds, bool* value)
{
    ...
    ImGuiWindow* child_window = g.CurrentWindow;
        ...
        PushStyleColor(ImGuiCol_WindowBg, GetColorU32(ui::colors::background));
        PushStyleColor(ImGuiCol_Border, GetColorU32(ui::colors::child::outline));
...
{
    SetCursorPos(GetCursorPos() + ImVec2(GetContentRegionMax().x - 194, 0));
...
bool ImGui::Tab(bool selected, const char* icon, const char* label, const char* hint,  const ImVec2& size_arg)
{
    ...
    it_anim->second.image = ImLerp(it_anim->second.image, selected || hovered ? ui::colors::main : ui::colors::text_disabled, g.IO.DeltaTime * 12.f);
    it_anim->second.text = ImLerp(it_anim->second.text, selected || hovered ? ui::colors::main : hovered ? ui::colors::text : ui::colors::text_disabled, g.IO.DeltaTime * 12.f);
    
    
    ImGui::GetForegroundDrawList()->AddText(image_bb.Min, GetColorU32(it_anim->second.image), icon);

    ImGui::GetForegroundDrawList()->AddShadowCircle(ImVec2(image_bb.Min.x + 12, image_bb.Min.y + 11), 5.f, GetColorU32(it_anim->second.image), 50.f, ImVec2(0, 0), 0, 12);

   // PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.text));
   // RenderTextClipped(bb.Min + ImVec2(38, 3), bb.Max, label, NULL, &label_size, ImVec2(0.0f, 0.0f), &bb);
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    it_anim->second.background = ImLerp(it_anim->second.background, IsItemActive() ? ui::colors::button::background_light : ui::colors::button::background, g.IO.DeltaTime * 6.f);
    it_anim->second.text = ImLerp(it_anim->second.text, IsItemHovered() ? ui::colors::main : ui::colors::text_disabled, g.IO.DeltaTime * 6.f);

    ImGui::GetWindowDrawList()->AddRectFilled(bb.Min, bb.Max, GetColorU32(it_anim->second.background), 3);
    ImGui::GetWindowDrawList()->AddRect(bb.Min, bb.Max, GetColorU32(ui::colors::button::outline_background), 3);

    ImGui::PushFont(font.spacegrotesk_medium[1]);

    ImGui::GetWindowDrawList()->AddText(
        ImVec2(
            bb.Min.x + (size.x - CalcTextSize(label).x) / 2,
            bb.Min.y + (size.y - CalcTextSize(label).y) / 2 - 1
        ),
...
bool ImGui::SubButtonEx(const char* label, bool selected, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    ...
    it_anim->second.background = ImLerp(it_anim->second.background, IsItemActive() ? ui::colors::button::background_light : ui::colors::button::background, g.IO.DeltaTime * 6.f);
    it_anim->second.text = ImLerp(it_anim->second.text,
        selected || IsItemHovered() ? ui::colors::main : ui::colors::text_disabled, g.IO.DeltaTime * 6.f);
...
bool ImGui::SubButton(const char* label, bool selected, const ImVec2& size_arg)
{
    return SubButtonEx(label, selected, size_arg, ImGuiButtonFlags_None);
}
...
bool ImGui::Tab(const char* icon, bool selected)
{
    ...
    it_hover->second = std::clamp(it_hover->second + (2.15f * GetIO().DeltaTime * (hovered ? 1.f : -1.f)), 0.0f, 0.0f);
    ...
    it_filled->second = std::clamp(it_filled->second + (2.15f * GetIO().DeltaTime * (selected ? 1.f : -1.5f)), it_hover->second, 1.f);
    it_filled->second *= GetStyle().Alpha;

    GetWindowDrawList()->AddRectFilled(ImVec2(bb.Min.x, bb.Min.y), ImVec2(bb.Max.x, bb.Max.y), IM_COL32(ui::colors::background.x * 255, ui::colors::background.y * 255, ui::colors::background.z * 255, int(255 * GetStyle().Alpha)), 8);
    ImVec2 text_size = CalcTextSize(icon);
    ImVec2 text_pos;
    if (icon == "\ue19b") // Gun
        text_pos.x = bb.Min.x + (bb.GetWidth() - text_size.x) * 0.5f + 1.f;
    else if (icon == "\xef\x81\xae") // Eye
        text_pos.x = bb.Min.x + (bb.GetWidth() - text_size.x) * 0.5f;
    else if (icon == "\xef\x82\xac") // Globe
        text_pos.x = bb.Min.x + (bb.GetWidth() - text_size.x) * 0.5f;
    else if (icon == "\xef\x80\x93") // Cog
        text_pos.x = bb.Min.x + (bb.GetWidth() - text_size.x) * 0.5f;
    else if (icon == "\xef\x80\x87") // User
        text_pos.x = bb.Min.x + (bb.GetWidth() - text_size.x) * 0.5f;

    text_pos.y = bb.Min.y + (bb.GetHeight() - text_size.y) * 0.5f + 3;

    ImU32 color = ImColor(96, 97, 102, int(255 * GetStyle().Alpha));
    if (selected)
        color = IM_COL32(ui::colors::main.x * 255, ui::colors::main.y * 255, ui::colors::main.z * 255, int(255 * it_filled->second));

    GetWindowDrawList()->AddText(text_pos, color, icon);
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis,
    ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v,
    ImDrawFlags flags)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    ...
    ImRect adjusted_bb_frame = bb_frame;

    float height_reduction = 8.0f;
    ImVec2 offset(-8, 4.0f); 

    if (size_avail_v == 392)
    {
        height_reduction = 16.0f;
        offset = ImVec2(-20.0f, 8.0f);
    }
 
    if (axis == ImGuiAxis_Y && adjusted_bb_frame.GetHeight() > height_reduction)
    {
        adjusted_bb_frame.Max.y -= height_reduction;
    }

    const float bb_frame_width = adjusted_bb_frame.GetWidth();
    const float bb_frame_height = adjusted_bb_frame.GetHeight();
    ...
    bool held = false;
    bool hovered = false;
    ItemAdd(adjusted_bb_frame, id, NULL, ImGuiItemFlags_NoNav);
    ...
    if (axis == ImGuiAxis_X)
    {
        float grab_x = ImLerp(bb.Min.x, bb.Max.x, grab_v_norm);
        grab_rect = ImRect(
            grab_x,
            adjusted_bb_frame.Min.y,
            grab_x + grab_h_pixels,
            adjusted_bb_frame.Max.y
        );
    }
    else
    {
        float grab_y = ImLerp(bb.Min.y, bb.Max.y, grab_v_norm);
        grab_rect = ImRect(
            adjusted_bb_frame.Min.x,
            grab_y,
            adjusted_bb_frame.Max.x,
            grab_y + grab_h_pixels
        );
    }

    window->DrawList->AddRectFilled(
        adjusted_bb_frame.Min - ImVec2(1, 1) + offset,
        adjusted_bb_frame.Max + ImVec2(1, 1) + offset,
        GetColorU32(ui::colors::background_light),
        8.0f
    );

    grab_rect.Translate(offset);

    float rounding = ImMin(8.0f, ImMin(grab_rect.GetWidth(), grab_rect.GetHeight()) * 0.5f);

    window->DrawList->AddRectFilled(
        grab_rect.Min,
        grab_rect.Max,
        GetColorU32(ui::colors::main),
        rounding
    );
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    ImVec2 pos = window->DC.CursorPos;
    ImVec2 box_pos = pos;
    ImVec2 text_pos = ImVec2(pos.x + box_size + style.ItemInnerSpacing.x + 4, pos.y + (box_size - label_size.y) * 0.5f);
    ImRect total_bb(pos, ImVec2(pos.x + box_size + style.ItemInnerSpacing.x + label_size.x, pos.y + ImMax(box_size, label_size.y)));
    ...
    state.box_color = ImLerp(state.box_color, *v ? ui::colors::main : ui::colors::checkbox::background, delta * speed);
    state.check_color = ImLerp(state.check_color, *v ? ImVec4(1, 1, 1, 1) : ImVec4(1, 1, 1, 0), delta * speed);
    state.text_color = ImLerp(state.text_color, *v ? ui::colors::text : hovered ? ui::colors::text : ui::colors::text_disabled, delta * speed);
    state.check_alpha = ImLerp(state.check_alpha, *v ? 1.0f : 0.0f, delta * speed);

    // Apply global alpha to box color
    ImVec4 final_box_color = state.box_color;
    final_box_color.w *= style.Alpha;

    ImU32 box_col = ImGui::ColorConvertFloat4ToU32(final_box_color);
    window->DrawList->AddRectFilled(box_pos, ImVec2(box_pos.x + box_size, box_pos.y + box_size), box_col, 3.0f);

    // Apply global alpha to outline
    ImVec4 outline_col = ui::colors::checkbox::outline_background;
    outline_col.w *= style.Alpha;
    window->DrawList->AddRect(box_pos, ImVec2(box_pos.x + box_size, box_pos.y + box_size), ImGui::ColorConvertFloat4ToU32(outline_col), 3.0f);

    // Draw animated check
    if (state.check_alpha > 0.01f) {
        const float thickness = 2.0f;
        ImVec2 p1 = ImVec2(box_pos.x + 3, box_pos.y + box_size * 0.5f);
        ImVec2 p2 = ImVec2(box_pos.x + box_size * 0.4f, box_pos.y + box_size - 4);
        ImVec2 p3 = ImVec2(box_pos.x + box_size - 3, box_pos.y + 3);

        ImVec4 final_check_col = state.check_color;
        final_check_col.w *= state.check_alpha * style.Alpha;

        ImU32 check_col = ImGui::ColorConvertFloat4ToU32(final_check_col);

        RenderCheckMark(window->DrawList, box_pos + ImVec2(box_size / 2, box_size / 2) - ImVec2(4.f, 4.f), check_col, 8.f);
    }

    // Draw label text with alpha
    ImVec4 final_text_color = state.text_color;
    final_text_color.w *= style.Alpha;

    ImU32 text_col = ImGui::ColorConvertFloat4ToU32(final_text_color);
    ImGui::PushFont(font.spacegrotesk_medium[1]);
    window->DrawList->AddText(ImVec2(text_pos.x, text_pos.y - 1), text_col, label);
    ImGui::PopFont();
...
{
    ImGui::ColorEdit4(label_picker, col, ImGuiColorEditFlags_NoSidePreview | ImGuiColorEditFlags_AlphaBar | ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_AlphaPreview | ImGuiColorEditFlags_NoLabel);
    SetCursorPos(GetCursorPos() - ImVec2(0, 28));
...
void ImGui::SeparatorEx(ImGuiSeparatorFlags flags, float thickness)
{
    ...
    if (flags & ImGuiSeparatorFlags_Vertical)
    {
        ...
        // Draw
        window->DrawList->AddRectFilled(bb.Min, bb.Max, GetColorU32(ui::colors::background_dark));
        if (g.LogEnabled)
            LogText(" |");
    }
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const float extra_height = 6.f;
    float offset_y = -4.0f;

    const ImRect frame_bb(window->DC.CursorPos + ImVec2(0, offset_y), window->DC.CursorPos + ImVec2(w, label_size.y + 20 + offset_y));
    const ImRect combo_bb(window->DC.CursorPos + ImVec2(0, 20 + offset_y), window->DC.CursorPos + ImVec2(w, label_size.y + 20 + extra_height + offset_y));
    const ImRect total_bb(frame_bb.Min, combo_bb.Max);

    ItemSize(total_bb, 4);
    if (!ItemAdd(total_bb, id, &combo_bb)) return false;
    ...
    auto& a = anim[id];
    a.text = ImLerp(a.text, popup_open ? ui::colors::text : hovered ? ui::colors::text : ui::colors::text_disabled, g.IO.DeltaTime * 6.f);
    a.combo = ImLerp(a.combo, popup_open ? ui::colors::main : hovered ? ui::colors::main : ui::colors::checkbox::circle_inactive, g.IO.DeltaTime * 6.f);

    GetWindowDrawList()->AddRectFilled(combo_bb.Min, combo_bb.Max, GetColorU32(ui::colors::combo::background), 2);
    GetWindowDrawList()->AddRect(combo_bb.Min, combo_bb.Max, GetColorU32(ui::colors::combo::outline_background), 2);

    ImGui::PushFont(font.spacegrotesk_medium[1]);
    PushStyleColor(ImGuiCol_Text, GetColorU32(ui::colors::text));
    GetWindowDrawList()->AddText(ImVec2(combo_bb.Min.x + 8, combo_bb.GetCenter().y - CalcTextSize(preview_value).y / 2), GetColorU32(ui::colors::text), preview_value);
    PopStyleColor();

    GetWindowDrawList()->AddText(frame_bb.Min, GetColorU32(a.text), label);
    ImGui::PopFont();
    ...
    return BeginComboPopup(popup_id, combo_bb, flags);
}
...
bool ImGui::BeginComboPopup(ImGuiID popup_id, const ImRect& bb, ImGuiComboFlags flags)
{
    ...
    PushStyleColor(ImGuiCol_PopupBg, GetColorU32(ui::colors::background));
    PushStyleColor(ImGuiCol_Border, GetColorU32(ui::colors::outline));
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, label_size.y + 20));
    const ImRect slider_bb(window->DC.CursorPos + ImVec2(0, 20), window->DC.CursorPos + ImVec2(w, label_size.y + 20));
    ...
    state.text = ImLerp(state.text, IsItemActive() ? ui::colors::text : hovered ? ui::colors::text : ui::colors::text_disabled, g.IO.DeltaTime * 6.f);

    // --- Background gradient ---
    ImColor bg_left = ui::colors::slider::background;
    ImColor bg_right = ImColor(
        ImClamp(bg_left.Value.x + 0.07f, 0.f, 1.f),
        ImClamp(bg_left.Value.y + 0.07f, 0.f, 1.f),
        ImClamp(bg_left.Value.z + 0.07f, 0.f, 1.f),
        bg_left.Value.w * style.Alpha // Apply alpha
    );

    bg_left.Value.w *= style.Alpha;

    GetWindowDrawList()->AddRectFilledMultiColorRounding(
        slider_bb.Min + ImVec2(0, 4), slider_bb.Max - ImVec2(0, 5),
        bg_left, bg_right, bg_right, bg_left,
        30.f
    );

    // --- Outline ---
    ImVec4 outline = ui::colors::slider::outline_background;
    outline.w *= style.Alpha;

    GetWindowDrawList()->AddRect(
        slider_bb.Min + ImVec2(0, 4), slider_bb.Max - ImVec2(0, 5),
        ImGui::ColorConvertFloat4ToU32(outline),
        30.f
    );
    ...
    {
        state.slow = ImLerp(state.slow, grab_bb.Min.x - frame_bb.Min.x, g.IO.DeltaTime * 10.f);

        float fill_x = state.slow + frame_bb.Min.x + 2;
        ImVec2 fill_min = slider_bb.Min + ImVec2(0, 4);
        ImVec2 fill_max = ImVec2(fill_x, grab_bb.Max.y - 4);

        ImColor fill_left = ui::colors::main;
        ImColor fill_right = ImColor(
            ImClamp(fill_left.Value.x + 0.07f, 0.f, 1.f),
            ImClamp(fill_left.Value.y + 0.07f, 0.f, 1.f),
            ImClamp(fill_left.Value.z + 0.07f, 0.f, 1.f),
            fill_left.Value.w * style.Alpha // Apply alpha
        );
        fill_left.Value.w *= style.Alpha;
        ...
        ImVec2 circle_center = ImVec2(frame_bb.Min.x + state.slow, slider_bb.GetCenter().y - 1);

        ImVec4 grabber = ui::colors::slider::grabber;
        grabber.w *= style.Alpha;

        GetWindowDrawList()->AddCircleFilled(circle_center, circle_radius, ImGui::ColorConvertFloat4ToU32(grabber), 16);
    }

    // --- Text ---
    ImGui::PushFont(font.spacegrotesk_medium[1]);

    ImVec4 text_col = state.text;
    text_col.w *= style.Alpha;

    const ImVec2 text_size = CalcTextSize(value_buf, NULL, true);
    ImVec2 text_pos = ImVec2(frame_bb.Max.x - text_size.x, frame_bb.Min.y);

    GetWindowDrawList()->AddText(text_pos, ImGui::ColorConvertFloat4ToU32(text_col), value_buf);
    GetWindowDrawList()->AddText(ImVec2(frame_bb.Min.x, frame_bb.Min.y), ImGui::ColorConvertFloat4ToU32(text_col), label);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImVec2 frame_size = CalcItemSize(size_arg, w, (is_multiline ? g.FontSize * 8.0f : label_size.y + style.FramePadding.y * 2.0f));

    const float offset_y = 4.0f; 

    ImVec2 label_pos = window->DC.CursorPos - ImVec2(0, offset_y);
    ImVec2 frame_bb_min = window->DC.CursorPos + ImVec2(0, label_size.y + style.ItemInnerSpacing.y) - ImVec2(0, offset_y);
    ImVec2 frame_bb_max = frame_bb_min + frame_size;
    ...
    const ImVec2 total_size = ImVec2(
        ...
        frame_size.y + label_size.y + style.ItemInnerSpacing.y
    );
    const ImRect total_bb(label_pos, label_pos + total_size);
    ...
    it_anim->second.text = ImLerp(it_anim->second.text, IsItemActive() ? ui::colors::text : hovered ? ui::colors::text : ui::colors::text_disabled, g.IO.DeltaTime * 6.f);

    if (!is_multiline)
    {
        GetWindowDrawList()->AddRectFilled(frame_bb.Min, frame_bb.Max, GetColorU32(ui::colors::input::background), 3);
        GetWindowDrawList()->AddRect(frame_bb.Min, frame_bb.Max, GetColorU32(ui::colors::input::outline_background), 3);
    }

    // Renderizar el label original
    ImGui::PushFont(font.spacegrotesk_medium[1]);
    GetWindowDrawList()->AddText(
        ImVec2(frame_bb.Max.x - w, frame_bb.Min.y - 22),
        GetColorU32(it_anim->second.text),
        label
    );
    ImGui::PopFont();
    ...
    ImVec2 draw_pos = is_multiline ? draw_window->DC.CursorPos : frame_bb.Min + ImVec2(5, (frame_size.y - CalcTextSize(hint).y) / 2);
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_selection)
        {
            ...
            ImU32 bg_color = ImColor(ui::colors::input::background_light.x, ui::colors::input::background_light.y, ui::colors::input::background_light.z, render_cursor ? 1.0f : 0.6f * 255); // FIXME: current code flow mandate that render_cursor is always true here, we are leaving the transparent one for tests.
            ...
            for (const ImWchar* p = text_selected_begin; p < text_selected_end; )
            {
                ...
                else
                {
                    ...
                    if (rect.Overlaps(clip_rect))
                        draw_window->DrawList->AddRectFilled(ImVec2(rect.Min.x, rect.Min.y + 1),
                            ImVec2(rect.Max.x, rect.Max.y - 1),
                            bg_color);
                }
        ...
        {
            ImGui::PushFont(font.spacegrotesk_medium[1]);
            ImU32 col = GetColorU32(is_displaying_hint ? ui::colors::text_disabled : ui::colors::main);
            draw_window->DrawList->AddText(g.Font, g.FontSize, ImVec2(draw_pos.x - draw_scroll.x, draw_pos.y - draw_scroll.y - 1), col, buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
            ImGui::PopFont();
        }
        ...
        if (render_cursor)
        {
            ...
            if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))
                draw_window->DrawList->AddLine(total_bb.Min + ImVec2(it_anim->second.slow + 1, 24), total_bb.GetBL() - ImVec2(-it_anim->second.slow, 24) + ImVec2(1, 20), GetColorU32(ui::colors::text_disabled));
    ...
    else
    {
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
        {
            ImGui::PushFont(font.spacegrotesk_medium[1]);
            ImU32 col = GetColorU32(is_displaying_hint ? ui::colors::text_disabled : ui::colors::main);
            draw_window->DrawList->AddText(g.Font, g.FontSize, ImVec2(draw_pos.x, draw_pos.y - 1), col, buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
            ImGui::PopFont();
        }
    }
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        if (ColorButton("##ColorButton", col_v4, flags))
        {
            if (!(flags & ImGuiColorEditFlags_NoPicker))
            {
                ...
                OpenPopup("picker");
                float offset_x = (g.LastItemData.Rect.GetWidth()) * 0.5f;
                SetNextWindowPos(g.LastItemData.Rect.GetBL() + ImVec2(offset_x, style.ItemSpacing.y));
            }
        }
        if (!(flags & ImGuiColorEditFlags_NoOptions))
            OpenPopupOnItemClick("context", ImGuiPopupFlags_MouseButtonRight);

        PushStyleColor(ImGuiCol_PopupBg, GetColorU32(ui::colors::picker::background));
        PushStyleColor(ImGuiCol_Border, GetColorU32(ui::colors::picker::outline_background));
    ...
    if (label != label_display_end && !(flags & ImGuiColorEditFlags_NoLabel))
    {
        ...
        ImGui::GetWindowDrawList()->AddText(window->DC.CursorPos + ImVec2(0, 1), GetColorU32(ui::colors::text_disabled), label);
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    // Render SV Square
    GetForegroundDrawList()->AddRectFilledMultiColorRounded(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size - 80), GetColorU32(ImGuiCol_PopupBg), col_white, hue_color32, hue_color32, col_white, 3, ImDrawCornerFlags_All);
    GetForegroundDrawList()->AddRectFilledMultiColorRounded(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size - 80), GetColorU32(ImGuiCol_PopupBg), 0, 0, col_black, col_black,3, ImDrawCornerFlags_All);
    ...
    it_anim->second.circle = ImLerp(it_anim->second.circle, value_changed_sv ? 4.f : 4.f, g.IO.DeltaTime * 20.f);

    ImVec2 center = it_anim->second.circle_move + bar_pos;
    float radius = it_anim->second.circle;
    ImVec2 min = center - ImVec2(radius, radius);
    ImVec2 max = center + ImVec2(radius, radius);

    GetForegroundDrawList()->AddRectFilled(min, max, user_col32_striped_of_alpha, 2.0f); // 4.0f is corner rounding

    GetForegroundDrawList()->AddRect(min + ImVec2(0, 1), max + ImVec2(0, 1), IM_COL32(50, 50, 50, int(150 * g.Style.Alpha)), 2.0f);

    GetForegroundDrawList()->AddRect(min, max, IM_COL32(230, 230, 230, int(255 * g.Style.Alpha)), 2.0f);
    ...
    if (alpha_bar)
    {
        ...
        GetForegroundDrawList()->AddRectFilledMultiColorRounded(bar1_bb.Min + ImVec2(0, 8), bar1_bb.Max - ImVec2(0, 5), GetColorU32(ImGuiCol_PopupBg), 0, 0, 0, 0, 3, ImDrawCornerFlags_All);
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg)
{
    ...
    const float default_size = GetFrameHeight();
    const ImVec2 pos = ImVec2(window->DC.CursorPos.x, window->DC.CursorPos.y + 1);
    const float width = GetContentRegionMax().x - GetStyle().WindowPadding.x;
    const ImRect rect(pos, pos + ImVec2(width, 18));
    const ImRect clickable(pos + ImVec2(width - 28, 3), pos + ImVec2(width, 16));
    ...
    float shrink = 4.0f;
    ImVec2 new_min = clickable.Min + ImVec2(shrink * 0.5f, 0.0f);
    ImVec2 new_max = clickable.Max - ImVec2(shrink * 0.5f, 0.0f);
    RenderColorRectWithAlphaCheckerboard(window->DrawList, new_min, new_max, GetColorU32(col_rgb), 3, ImVec2(0.f, 0.f), 3);
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    static std::map<ImGuiID, float> offset_anim;
    auto& a = anim[id];
    float& offset = offset_anim[id];

    // Hover o selected activa color y alpha, pero offset solo si seleccionado
    const bool active_anim = selected || hovered;

    // Color animado
    ImVec4 target_color = selected ? ui::colors::main : (hovered ? ui::colors::text : ui::colors::text_disabled);
    a.text = ImLerp(a.text, target_color, g.IO.DeltaTime * 6.f);

    // Alpha para efectos opcionales (como la barra)
    float target_alpha = active_anim ? 1.0f : 0.0f;
    a.bg_alpha = ImLerp(a.bg_alpha, target_alpha, g.IO.DeltaTime * 10.f);

    // Offset solo si está seleccionado
    float target_offset = selected ? 6.0f : 0.0f;
    offset = ImLerp(offset, target_offset, g.IO.DeltaTime * 10.0f);

    // Línea lateral solo si seleccionado
    if (selected)
    {
        const float bar_width = 2.0f;
        const float bar_height = label_size.y * 0.6f;
        const float spacing = 2.0f;

        ImVec2 actual_text_pos = text_min + ImVec2(offset - 2, 0.0f);
        float bar_y = actual_text_pos.y + (label_size.y - bar_height) * 0.5f + 2;

        ImVec2 bar_min = ImVec2(actual_text_pos.x - spacing - bar_width, bar_y);
        ImVec2 bar_max = ImVec2(bar_min.x + bar_width, bar_y + bar_height);

        window->DrawList->AddRectFilled(bar_min, bar_max, GetColorU32(a.text), 2.0f);
    }

    // Render del texto
    ImVec2 text_offset = ImVec2(offset, 0.0f);
    PushFont(font.spacegrotesk_medium[1]);
    PushStyleColor(ImGuiCol_Text, GetColorU32(a.text));
    RenderTextClipped(text_min + text_offset, text_max + text_offset, label, NULL, &label_size, style.SelectableTextAlign, &bb);
    PopStyleColor();
    PopFont();
